 ...
 */
function applyJoinProfileNudge(session, tasteProfile) {
  if (!session || !tasteProfile || !tasteProfile.genreCounts) return;
  if (!session._joinGenreNudge || typeof session._joinGenreNudge !== 'object') {
    session._joinGenreNudge = emptyGenreCounts();
  }
  const norm = normalizeVector(tasteProfile.genreCounts);
  if (!norm) return;
  const step = 0.06;
  for (const g of GENRE_BUCKETS) {
    const add = (norm[g] || 0) * step;
    session._joinGenreNudge[g] = Math.min(0.55, (Number(session._joinGenreNudge[g]) || 0) + add);
  }
}
...
function computeFullSessionVibe(session, tasteProfiles) {
  ...
  const joinNudge = session._joinGenreNudge && typeof session._joinGenreNudge === 'object' ? session._joinGenreNudge : null;
  if (joinNudge) {
    for (const g of GENRE_BUCKETS) {
      combined[g] += Math.max(0, Number(joinNudge[g]) || 0);
    }
  }
