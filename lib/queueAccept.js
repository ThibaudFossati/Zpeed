...
function sortQueueForUi(session) {
  ...
  const isLowPriorityAuto = t => !!(t && (t.autoDJ === true || t.aiSuggested === true));
  session.queue.sort((a, b) => {
    const softA = Number(a.softInfluenceBonus) || 0;
    const softB = Number(b.softInfluenceBonus) || 0;
    const sa = (Number(a.likes) || 0) - (Number(a.skips) || 0) + softA;
    const sb = (Number(b.likes) || 0) - (Number(b.skips) || 0) + softB;
    if (sb !== sa) return sb - sa;
    const ua = !isLowPriorityAuto(a);
    const ub = !isLowPriorityAuto(b);
